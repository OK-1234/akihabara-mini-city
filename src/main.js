...
try {
  ...
  console.info('街の骨格 試作2号', {tanukiHeight:player.height,walkSpeed:3,dashSpeed:4.8});
  let previous;
  renderer.setAnimationLoop(time=>{const dt=previous===undefined?0:Math.min((time-previous)/1000,.05);previous=time;player.update(dt);rig.follow(player.position);renderer.render(scene,rig.camera);});
